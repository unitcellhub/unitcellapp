...
FAQ_ENGINEERING_CONSTANTS = r"""

The constitutive model for an orthotropic material is
...
where $\epsilon_{ii}$ and $\gamma_{ij}$ are engineering strentch and shear strains, respectively, $\sigma_{ij}$ are engineering stresses,
$E_i$ are elastic moduli in the *i*-direction, and $G_{ij}$ are shear moduli on the *i*-face in the *j*-direction.
The convention for Poisson's ratio $\nu_{ij}$ is defined to characterize the transverse strain in the *j*-direction when the material is stressed in the *i*-direction ($\nu_{ij} = -\epsilon_{jj}/\epsilon_{ii}$).
The opposite convention is occationally used in the literature, so pay special attention to the definitions when applying the constants.
Additionally, note that, unlike isotropic materials, Poisson's ratio for orthotropic materials is not restricted to the range \[-1, 0.5\].

"""

FAQ_CUSTOM = """
UnitcellApp is built upon simulated property data generated by [UnitcellEngine](https://github.com/unitcellhub/unitcellengine) and stored in [UnitcellDB](https://github.com/unitcellhub/unitcelldb), which are additional modules found in the [UnitcellHub](https://github.com/unitcellhub) framework. 
Custom unit cell geometries can be defined and parametrically swept using UnitcellEngine to characterize their mechanical and thermal properties through the same homogenization-based finite element analysis pipeline used to generate the existing database.

Once characterized, these custom unit cell properties can be incorporated into UnitcellApp in two ways: (1) deploy a local instance of UnitcellApp with your expanded database, or (2) submit a request to have promising unit cell types added to the public database. 
For instructions on local deployment and custom database generation/integration, visit the [UnitcellApp GitHub repository](https://github.com/unitcellhub/unitcellapp). 
To request new unit cell types for the public database, please submit an [issue on the GitHub repository](https://github.com/unitcellhub/unitcellapp/issues) describing the geometry and application requirements.
"""
                ...
                [
                    ...
                    dbc.Accordion(
                        [
                            ...
                            dbc.AccordionItem(
                                [dcc.Markdown(FAQ_ENGINEERING_CONSTANTS, mathjax=True)],
                                title="What is the constitutive relationship for a homogenized orthotropic material?",
                            ),
                            dbc.AccordionItem(
                    ...
                    dbc.Accordion(
                        [
                            ...
                            dbc.AccordionItem(
                                [dcc.Markdown(FAQ_CUSTOM)],
                                title="What if I'm interested in a unitcell that doesn't exist in UnitcellApp?",
                            ),
                            dbc.AccordionItem(
