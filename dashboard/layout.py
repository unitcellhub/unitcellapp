...
try:
    ...
    with tables.open_file(database, "r") as h5file:
        ...
        OPTIONS_MOD = copy(_DEFAULT_OPTIONS)
    ...
    for form, unitcells in _DATA.items():
        for unitcell, values in unitcells.items():
            ...
            params = ["length", "width", "height", "thickness"]
            ...
            tosave = blosc.compress(
                pickle.dumps(
                    {
                        "data": _DATA[form][unitcell],
                        "surrogate": _SURROGATE[form][unitcell],
                    }
                )
            )
            with open(CACHE / Path(f"database_{form}_{unitcell}.pkl"), "wb") as f:
                f.write(tosave)
...
def _customQoi(ind):
    ...
    label = html.Div(
        ...
        title=("Label used to identify the custom QOI " "throughout the dashboard."),
    )
    equation = html.Div(
        dbc.Input(
